window.SURVEY_CONFIG = Object.freeze({
  // 完成 Apps Script 部署後，填入 /exec 網址。未設定時僅供預覽，不會送出。
  endpoint: 'https://script.google.com/macros/s/AKfycbwd7OQAFSFkipbDrCZj8nQwwObYDOFN2uTGoXKqG9DpyXCYfY_SsaLJA_fXPHAKCO3J/exec',
  organizer: '承辦單位：健康樂活與智慧醫療照護聯盟', // 主辦／資料管理單位
  contact: '聯絡信箱：SmartMedHealth@itri.org.tw', // 撤回與資料問題的 Email 或電話
  retention: '用途說明：資料僅供研究統計分析使用', // 例如：蒐集截止後 12 個月刪除原始回覆
  purpose: '評估台灣社區基層診所加入協作中心的條件、支援需求與效率期待，供單一中心規模及服務規劃使用。',
  consentVersion: '2026-09-23-v1',
  surveyVersion: 'clinic-survey-1.0'
});
